...
const ClaimModal: FC<ClaimModalProps> = ({ open: defaultOpen, rewards, disabled, onClose }) => {
  ...
  const claims: (number | number[])[][] = useMemo(() => {
    const claimList: Record<string, number[]> = {};
    (deposits || []).forEach((d: Deposit) => {
      if (!claimList[d.roundId]) {
        claimList[d.roundId] = [d.indice]
      } else {
        claimList[d.roundId].push(d.indice)
      }
    });

    return Object.keys(claimList).map((k:string) => [+k, claimList[k]]).concat(rewards)
  }, [deposits, rewards])
  ...
  const handleClaimReward = async () => {
    ...
    try {
      ...
      console.log(protocolFeeOwed, claims)
