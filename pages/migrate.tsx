...
const BridgePage = () => {
  ...
  useEffect(() => {
    if (nfteBalance?.result) {
      const val = formatUnits(BigNumber.from(bridgePercent).mul(nfteBalance?.result?.toString()).div(100).toString(), 18)
      setValueEth(val)
    }
  }, [debouncedPercent])
            ...
            <Flex
              ...
              <Box css={{
                ...
                >
                  ...
                  <Text>{`Estimated Fee : ${formatEther(BigNumber.from(estimateFee?.result?.[0]?.toString() || 300000000000000).div(100).mul(200).toString()) || '-'}`}</Text>
